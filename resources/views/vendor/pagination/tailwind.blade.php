...
@if ($paginator->hasPages())
    <nav role="navigation" aria-label="Pagination Navigation"
        ...
        @foreach (range(max(1, $paginator->currentPage() - 1), min($paginator->lastPage(), $paginator->currentPage() + 1)) as $page)
            @if ($page == $paginator->currentPage())
                <span class="px-3 py-1 border border-[#1b1e4a] rounded-lg bg-[#1b1e4a] text-white">{{ $page }}</span>
            @else
                <a href="{{ $paginator->url($page) }}"
                    class="px-3 py-1 border border-gray-300 rounded-lg text-[#1b1e4a] hover:bg-[#1b1e4a] hover:border-[#1b1e4a] hover:text-white transition">{{ $page }}</a>
            @endif
        @endforeach
        ...
        @if ($paginator->currentPage() < $paginator->lastPage() - 2)
            <a href="{{ $paginator->url($paginator->lastPage()) }}"
                class="px-3 py-1 border border-gray-300 rounded-lg text-[#1b1e4a] hover:bg-[#1b1e4a] hover:border-[#1b1e4a] hover:text-white transition">{{ $paginator->lastPage() }}</a>
        @endif
