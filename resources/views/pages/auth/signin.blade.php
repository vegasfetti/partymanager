...
@section('title', 'Вход | ПАТИМЕНЕДЖЕР')
...
@section('content')
    ...
    <main class="container min-h-screen relative px-4 sm:px-6 lg:px-8">
        ...
        <form method="post" action="{{ route('action.signin') }}"
            ...
            @csrf
            <h1 class="font-semibold text-3xl sm:text-4xl">Авторизация</h1>
            <p class="font-light text-base sm:text-lg opacity-50 pt-2.5 pb-6 sm:pb-7.5">С возвращением</p>
            ...
            <div class="inputs flex flex-col gap-2.5">
                ...
                @enderror
                @error('forgot')
                    <a href="{{ route('password.request') }}" class="text-right w-full text-[#FFD700] text-sm">Забыли
                        пароль?</a>
                @enderror
                ...
                <button
                    class="waiting bg-[#FFD700] font-semibold text-[#1C1F4C] mt-5 px-3.5 sm:px-4 py-2.5 sm:py-3 border border-[#0000001a] rounded-xl cursor-pointer w-full sm:w-auto text-base sm:text-sm">Войти</button>

                <p class="text-sm sm:text-base text-gray-500 text-center pt-5">Ещё нет аккаунта? <a
                        class="text-[#FFD700] underline" href="{{ route('signup') }}">Зарегистрироваться</a></p>
            </div>
        </form>
